        ...
        for epoch in epochs:
            ...
            for batch in np.array_split(np.random.permutation(self.data_size), max(int(self.data_size / self.batch_size), 1)):
                ...
                def sim_loss():
                    ...
                    Z_sim = (At @ z0.reshape(batch.shape[0], self.dim_K, 1)).reshape(-1, self.dim_K)
            ...
            # if self.debug: print('Epoch', epoch + 1, '|', 'loss: ', batch_loss.item())
            epochs.set_description("Loss: %1.4e" % batch_loss.item())

            if (epoch % 1000 == 0):
                for param in self.optimizer.param_groups:
                    param['lr'] *= self.lr_decay
...
class SimErrorOptimizerWithInput(object):
